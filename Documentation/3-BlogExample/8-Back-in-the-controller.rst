...
variable :php:`$this->view`.

With the method :php:`assign()` we "bind" the array with our blogs to the variable
name "blogs" of the TemplateView. It can be addressed with this name in the
template. The method :php:`render()` of the TemplateView starts the generation of the
HTML code.
...
initialization to the method :php:`initializeAction()`. This method is called by
...
method :php:`initializeIndexAction()`. The string *IndexAction* needs to be replaced
...
:php:`render()`. If the action does not return a result with :php:`return $content` by
itself (either because the call is missing or returns NULL), Extbase
automatically calls the method :php:`render()`.

.. note::

	This automatism can be confusing, because you have to specify :php:`return '';`
